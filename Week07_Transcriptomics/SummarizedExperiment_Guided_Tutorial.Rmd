...
# what are the rows...?
## NULL meaning there are no rows?
...
colnames_mask <- factor(colnames_mask)
...
```{r Exercise 2.6}
age_NA_mask <- !is.na(rna_clinical$age_at_index) # here we've subset the age at index and removed any NA values. we've created a mask in this provess called "age_NA_mask"
  
rna_clinical <- rna_clinical[age_NA_mask,]  # now we have applied the mask to the rows of rna_clinical and have replaced the original age_at_index values with the cleaned ones. -> look at boolean masking to understand how this works
