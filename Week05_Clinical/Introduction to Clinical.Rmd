...
```{r Exercise 1.7}
mtcars[c("Duster 360", "Lincoln Continental", "Maserati Bora"), "hp"] # write code here
```
...
```{r Exercise 1.10}
mtcars_copy <- mtcars[, c(1:8,10:12)]
head(mtcars_copy)
```
...
```{r Exercise 1.11}
mtcars_copy <- mtcars[, -9]# write code here
head(mtcars_copy)
```
...
```{r Exercise 1.12}
mtcars_copy # write code here
# the "am" column has been dropped :)
```
...
clin_query <- GDCquery(project = "TCGA-BRCA" #Fill in correct command
,
...
clinic <- clinical.BCRtab.all$clinical_patient_brca[-c(1,2),] #Fill in correct command
...
```{r}
write.csv(clinic, "/home1/arsalans/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/arsalans/analysis_data/brca_clinical_data.csv")
head(clinic)
```
...
rownames(brca_clinical_data)
colnames(brca_clinical_data)
# 1. there are 1097 rows, meaning there are 1097 patients. there are 112 columns, looking for various measures.

head(brca_clinical_data$gender)
# 2. according to the head() data of brca data set, our data type of $gender is characteristic with patients either being "female" or "male"

brca_clinical_data$birth_days_to[ clinic$Tumor_Sample_Barcode == "TCGA-4H-AAAK" ]
# 3. the age of the 5th patient is 50 years old (according to birth_days_to)

brca_clinical_data[1, "birth_days_to"]
brca_clinical_data[1, "last_contact_days_to"]
# 4. the 1st patient was around 45 on her last check up and is currently around 50 years old. ((20211-3767)/365) = ~45 years

```

# 1. How many rows are there? How many columns? View the column and row names. What two commands should you use?

```{r Exercise 3.2}
rownames(brca_clinical_data)
colnames(brca_clinical_data) # write code here
# these will both returnt he names of each column and row. The rows in this data set are numbers.
```

View the vital status column. What two notations can be used to view columns? 

```{r Exercise 3.3}
brca_clinical_data$vital_status
brca_clinical_data[, "vital_status"] # write code here
# these are two variations of the same output.
```
...
brca_clean <- subset(brca_clinical_data, 
                     birth_days_to != "[Not Available]" & 
                     lymph_nodes_examined_count != "[Not Available]")

age <- abs(as.numeric(brca_clean$birth_days_to)) / 365

lymph_nodes_examined_num <- as.numeric(brca_clean$lymph_nodes_examined_count)

# create plot here
plot(age, lymph_nodes_examined_num,
     main = "Age vs Number of Lymph Nodes Examined",
     xlab = "Current Age (years)",
     ylab = "Number of Lymph Nodes (count)",
     col = "blue", pch = 19)
...
brca_clean2 <- subset(brca_clinical_data, 
                     birth_days_to != "[Not Available]" & 
                     race != "[Not Available]" & race != "[Not Evaluated]")

age2 <- abs(as.numeric(brca_clean2$birth_days_to)) / 365
race_data <- (brca_clean2$race)
# create plot here
boxplot(age2 ~ race_data,
        main = "Age by Race",
        xlab = "Age",
        ylab = "Race",
...
```{r}
male_mask <- is.na(clinic$male)
```

3) Create a mask to select for patients under the age of 30 OR over 60 at diagnosis. 
```{r}
...
```{r}
age_NA_mask <- !is.na(clinic$age_at_diagnosis == "[Not Available]")
...
# y-axes represents time in days that a category of patient has survived for.

# 2. What does the x axis represent?
# The survivability probability, meaning how likely is it that at that time interval is the patient to be alive. 

# 3. What age group appears to have the greatest rate of survival?
# The young category has the greatest rate of survival according to the KM plot since it stays at a high survivability probability for longer than the middle and old age groups. There are less young patients, however.

# 4. What age group appears to have the worst rate of survival? 
# The old patient group has the worst rate of survival because is generally has the worse survivablility curve in regards to the other three groups and the curve stays below both of them for the majority of the curve.

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
# Assuming the time x-axis is looking at days the patient has survived, the 15-year survival rate for older patients is aronud 26%. This is the very end of the green line where it stays at 26% until it ends. This is because 15-year survival is at the ~5,400 mark on the x-axis.
...
# The p-val is 0.2, which is not a good p-value to evaluate for significance. If we had a good value, we would be looking for something below 0.05, which is 5%. This means that there is a 20% chance that the data is due to random chance, and therefore we cannot accept that alternative hypothesis, meaning age and breast cancer survival are not significantly correlated.
